#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""V0.2.6.1: image/screenshot feed with Mac-local Apple Vision OCR."""
...
def injected_index():
    ...
    if "ocr.css?v=0261" not in html:
        html = html.replace("</head>", '<link rel="stylesheet" href="/ocr.css?v=0261" /></head>')
    if "ocr.js?v=0261" not in html:
        html = html.replace("</body>", '<script src="/ocr.js?v=0261"></script></body>')
    return html
...
if callable(old_progress):
    def progress_v0261():
        ...
        return d
    core.get_progress = progress_v0261
...
def post_v0261(self):
    ...
    if path == "/api/ocr":
        try:
            ...
            core.log("ERROR V0.2.6.1 OCR: " + traceback.format_exc())
            return self._json({"ok": False, "error": str(e)}, 500)
    return old_post(self)

core.Handler.do_POST = post_v0261
