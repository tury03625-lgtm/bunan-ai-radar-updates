#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""V0.2.6.3: Mac-local Apple Vision OCR bridge fix."""
from pathlib import Path
import base64, importlib.util, json, subprocess, sys, traceback, urllib.parse, uuid
...
def injected_index():
    ...
    if "ocr.css?v=0263" not in html:
        html = html.replace("</head>", '<link rel="stylesheet" href="/ocr.css?v=0263" /></head>')
    if "ocr.js?v=0263" not in html:
        version_fix = """<script src="/ocr.js?v=0263"></script>
<script>
document.addEventListener('DOMContentLoaded',function(){
  var b=document.querySelector('.brand-sub'); if(b) b.textContent='不楠先生 · V0.2.6.3';
  var c=document.querySelector('#current-version'); if(c) c.textContent='V0.2.6.3';
});
</script>"""
        html = html.replace("</body>", version_fix + "</body>")
    return html
...
def ocr_request(body):
    ...
    try:
        ...
        result.update({"ok": True, "name": name, "chars": len(result["text"]),
                       "local_only": True, "image_retained": False})
        return result
    finally:
        try:
...
def get_v0263(self):
    ...
    if path == "/api/health":
        return self._json({"ok": True, "product": "bunan-ai-radar",
                           "name": "不楠先生AI招商雷达预警评估体系系统",
                           "version": TARGET_VERSION, "ui_contract": "interactive-v1",
                           "ocr_engine": "apple-vision-jxa-v2"})
    ...
    if path == "/api/ocr/status":
        ...
        return self._json({"ok": True, "local_only": True, "vision_jxa": available,
                           "vision_swift_fallback": False, "max_image_mb": 10,
                           "formats": ["PNG", "JPG", "JPEG"],
                           "engine_version": "jxa-v2",
                           "privacy": "原图只在Mac本地临时处理，OCR完成后立即删除；不会上传给DeepSeek。"})
    return old_get(self)
core.Handler.do_GET = get_v0263
...
def post_v0263(self):
    ...
    if path == "/api/ocr":
        try:
            ...
            core.log("ERROR V0.2.6.3 OCR: " + traceback.format_exc())
            return self._json({"ok": False, "error": str(e)}, 500)
    return old_post(self)
core.Handler.do_POST = post_v0263
